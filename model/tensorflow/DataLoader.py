...
class DataLoaderDisk(object):
    ...
    def next_batch(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            #scipy.misc.imsave(str(i)+".png", image)
            seed = int(np.random.random(1)*10000.)
            print(seed)
            image = skimage.util.random_noise(image, mode='s&p', seed=seed, clip=True)

            #scipy.misc.imsave(str(i)+"anoise.png", image)

            if self.randomize:
                angleToRotate = np.random.random(1)*180-90 #between -90 and 90
                image = scipy.misc.imrotate(image, angleToRotate)
                off = int(-np.abs(27.0/45.0*np.abs(angleToRotate)-27) + 27) #two spikes
                image = image[off:len(image)-(off), off:len(image)-(off), :]
            ...
            newimage = image[offset_h:offset_h+self.fine_size, offset_w:offset_w+self.fine_size, :]
            images_batch[i, ...] = newimage 
            ...
            #scipy.misc.imsave(str(i)+"new.png", newimage)
